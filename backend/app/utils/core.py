...
def signJWT(user_id: str):
    payload = {"user_id": user_id, "expire": time.time() + 600}
    token = jwt.encode(
        payload, settings.JWT_SECRET_KEY, algorithms=[settings.ALGORITHM]
    )
    ...
    try:
        for k, v in parsed_schema.items():
            if isinstance(v, list) and len(v) and is_pydantic(v[0]):
                parsed_schema[k] = [
                    item.Meta.model(**schema_to_model(item)) for item in v
                ]
            elif is_pydantic(v):
                parsed_schema[k] = v.Meta.model(**schema_to_model(v))
    except AttributeError:
        raise AttributeError(
            f"found nested pydantic model in {schema.__class__} but Meta.model was not specified."
        )
